...
import ApprovedBloodStorage from "../approved/index";
import VerifiedBloodStorage from "../verify/index";
...
const BloodStorageList = () => {
    ...
    // Sử dụng dữ liệu tĩnh thay vì state và fetch API
    // const [bloodTypes, setBloodTypes] = useState([]); // <-- Bỏ dòng này
    // const [bloodComponents, setBloodComponents] = useState([]); // <-- Bỏ dòng này
    const bloodTypes = STATIC_BLOOD_TYPES; // <-- Sử dụng dữ liệu tĩnh
    const bloodComponents = STATIC_BLOOD_COMPONENTS; // <-- Sử dụng dữ liệu tĩnh
    ...
    // Lấy token và role từ hook tiện ích
    ...
    const fetchBloodStorages = async () => {
        ...
        try {
            ...
            // Kiểm tra xem có bất kỳ filter tìm kiếm nào được chọn không
            ...
            if (isSearchingByTypeOrComponent) {
                ...
                config.params.status = undefined; // Đảm bảo không gửi param status
    ...
    // useEffect này không còn cần thiết để tải dữ liệu tĩnh
    // useEffect(() => {
    //     fetchBloodTypes();
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.filterSection}>
                ...
                {/* Phần mới cho search theo Type và Component */}
                <div className={styles.filterGroup}>
            ...
            {isModalOpen && selectedStorage && (
                <Modal onClose={handleCloseModal}>
                    <div className={styles.modalContent}>
                        ...
                        <ApprovedBloodStorage
                            storageId={selectedStorage.id}
                            onApprovedSuccess={handleApprovedSuccess}
                            currentStatus={selectedStorage.bloodStatus}
                            userRole={userRole}
                        />

                        <VerifiedBloodStorage
                            storageId={selectedStorage.id}
                            currentStatus={selectedStorage.bloodStatus}
                            verifiedStatus={selectedStorage.verifiedStatus}
